...
class Qwen4ExpPinnedHostEmbedding(VocabParallelEmbedding):
    ...
    def __init__(self, embedding: VocabParallelEmbedding) -> None:
        ...
        # soc-poc GB10 patch: on unified-memory parts, pinned host memory and
        # GPU memory are the same 121 GB pool, so pinning this ~48 GiB table
        # OOMs the box before the KV pool exists. With SGLANG_QWEN4_PLE_MMAP_DIR
        # set, the table becomes a MAP_SHARED file-backed tensor instead: the
        # kernel can evict its pages under pressure, and the gather kernel reads
        # it through the same host pointer -- coherent unified memory makes a
        # pageable pointer as GPU-readable as a pinned one. Everything else
        # (class identity, weight loading, gather) is unchanged native code.
        _mmap_dir = os.environ.get("SGLANG_QWEN4_PLE_MMAP_DIR", "").strip()
        if _mmap_dir:
            os.makedirs(_mmap_dir, exist_ok=True)
            _table_path = os.path.join(_mmap_dir, "qwen4_ple_table.bin")
            _backing = torch.from_file(
                _table_path,
                shared=True,
            ...
            cpu_weight = nn.Parameter(
                _backing.view(source_weight.shape), requires_grad=False
            )
            logger.info(
                "Qwen4 PLE table is file-backed (MAP_SHARED) at %s: %.2f GiB",
                _table_path,
                source_weight.numel() * source_weight.element_size() / (1 << 30),
            )
        else:
            cpu_weight = nn.Parameter(
                torch.empty(
